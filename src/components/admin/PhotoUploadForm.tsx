...
const PhotoUploadForm = ({ onSubmit }: PhotoUploadFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Input
            ...
            value={form.caption}
            onChange={handleChange}
            placeholder="Короткая подпись под фото"
          />
        </div>
          ...
          <img
            ...
            onError={(e) =>
              ((e.target as HTMLImageElement).src =
                "https://via.placeholder.com/150?text=Неверный+URL")
            }
          />
          {form.caption && (
            <p className="text-sm mt-2 text-gray-600 italic">{form.caption}</p>
          )}
        </div>
      )}
      ...
      <Button
        type="submit"
        className="bg-glowb-blue-deep hover:bg-glowb-blue mt-4"
      >
